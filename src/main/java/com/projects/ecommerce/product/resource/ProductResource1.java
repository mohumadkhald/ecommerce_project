...
//		List<String> colors = color != null ? color.stream().map(String::toLowerCase).toList() : null;
//		Sort sort = Sort.by(sortDirection.equals("desc") ? Sort.Direction.DESC : Sort.Direction.ASC, sortBy);
//		Page<ProductDto> products = productService.getProductsByCategoryNameAndProdcutNameAndFilters(category, productNmae, colors, minPrice, maxPrice, uppercaseSizes, available,page, pageSize, sort); // Convert size to uppercase
//		return ResponseEntity.ok(products);
//	}
//
//
